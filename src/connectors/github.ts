...
export class GitHubConnector implements Connector {
  ...
  private async fetchIssues(): Promise<Document[]> {
    ...
    while (hasMore && page <= 10) {
      const response = await fetch(
        `${this.baseUrl}/repos/${this.config.owner}/${this.config.repo}/issues?state=all&page=${page}&per_page=100`,
        {
          headers: {
            'Authorization': `token ${this.config.token}`,
            'Accept': 'application/vnd.github.v3+json'
  ...
  private async fetchPRs(): Promise<Document[]> {
    ...
    while (hasMore && page <= 10) {
      const response = await fetch(
        `${this.baseUrl}/repos/${this.config.owner}/${this.config.repo}/pulls?state=all&page=${page}&per_page=100`,
        {
          headers: {
            'Authorization': `token ${this.config.token}`,
            'Accept': 'application/vnd.github.v3+json'
  ...
  private async fetchDiscussions(): Promise<Document[]> {
    const documents: Document[] = [];
    ...
    const data = await response.json() as { data?: { repository?: { discussions?: { nodes: GitHubDiscussion[] } } } };
